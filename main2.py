                ...
                for paragraph in shape.text_frame.paragraphs:
                    for run in paragraph.runs:
                        ...
                        if any(placeholder in text for placeholder in ['.PESSOA', '.OFICINA1', '.OFICINA2', '.HORAS', '.OPERAÇÃO', '.MUNICIPIO', '.PERIODO', '.COORDENAÇÃO']):
                            ...
                            if pd.notnull(row['Periodo']) and not isinstance(row['Periodo'], str):
                                text = text.replace('.PERIODO', row['Periodo'].strftime('%d/%m/%Y'))  # Formatar data
                            else:
                                text = text.replace('.PERIODO', row['Periodo'] if pd.notnull(row['Periodo']) else '')
...
for pdf_file in tqdm(pdf_files, desc="Convertendo PDF para imagens"):
    ...
    for page_num in range(len(doc)):
        ...
        pix = page.get_pixmap(dpi=600)  # Ajustar a resolução para 600 DPI
        image_path = pdf_file.replace('.pdf', f'_{page_num}.png')
        pix.save(image_path)
        image_files.append(image_path)
    doc.close()  # Fechar o documento PDF

# Criar PDFs individuais para cada certificado
for i, image_file in enumerate(tqdm(image_files, desc="Criando PDFs individuais")):
    pessoa = df.iloc[i]['Pessoa']
    oficina1 = df.iloc[i]['Oficina 1'].replace(':', ' ')
    pdf_path = os.path.join(output_directory, f'{oficina1}_{pessoa}.pdf')
...
print(f"\nCertificados salvos em: {output_directory}\n")